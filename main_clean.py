...
def load_config() -> Dict[str, Any]:
    ...
    if "backup_path" in raw and "global" not in raw:
        migrated = default_config()
        migrated["global"]["backup_path"] = raw.get("backup_path", "./backups")
        migrated["global"]["screenshot_path"] = raw.get("screenshot_path", "./screenshots")
        migrated["global"]["record_duration"] = raw.get("record_duration", 30)
        migrated["global"]["scrcpy_path"] = raw.get("scrcpy_path", "scrcpy")
        migrated["version"] = raw.get("version", "3.0")
        migrated["language"] = raw.get("language", "de")
        migrated["theme"] = raw.get("theme", "light")
        migrated["auto_update_check"] = raw.get("auto_update_check", True)
        migrated["license_check_enabled"] = raw.get("license_check_enabled", False)
        return migrated
...
def check_license_safe(config: Dict[str, Any]) -> bool:
    if not config.get("license_check_enabled", False):
        logger.debug("Lizenzprüfung deaktiviert.")
        return True
...
def ensure_runtime_dirs(config: Dict[str, Any]) -> None:
    Path(config["global"]["backup_path"]).mkdir(parents=True, exist_ok=True)
    Path(config["global"]["screenshot_path"]).mkdir(parents=True, exist_ok=True)
...
def run_dependency_checks() -> None:
    ...
    if not results.get("adb"):
        ...
        print_header("POSEIDON", "v3.0 - ADB Power Tool")
        print(f"{fg.RED}KRITISCHER FEHLER: 'adb' wurde nicht gefunden!{style.RESET}")
        print("Stellen Sie sicher, dass ADB installiert ist und im System-Pfad liegt.")
        sys.exit(1)

    if warnings:
        logger.warning(f"Abhängigkeits-Warnungen gefunden: {', '.join(warnings)}")
        print_header("POSEIDON", "Abhängigkeits-Warnung")
        for warning in warnings:
            print(f"{fg.YELLOW}[!] {warning}{style.RESET}")
        print("-" * 50)
        if sys.stdin.isatty():
            if not confirm("Möchten Sie trotzdem fortfahren?"):
                sys.exit(0)
        else:
            print("Nicht-interaktiver Modus erkannt, fahre fort...")
...
def settings_menu(config: Dict[str, Any], device_manager: DeviceManager) -> None:
    while True:
        ...
        choice = menu_prompt(get_text("choose_option"), range(0, 8))
        if choice == 0:
            break
        elif choice == 1:
            ...
            if langs:
                ...
                if new_lang in langs:
                    ...
                    print(get_text("invalid_language"))
        elif choice == 7:
...
def main() -> None:
    color_init()
    logger.info("Poseidon v3.0 wird gestartet...")
    ...
    while running:
        ...
        choice = menu_prompt(get_text("choose_category"), range(0, 21))
